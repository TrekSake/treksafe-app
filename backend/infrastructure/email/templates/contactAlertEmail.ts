export type ContactAlertEmailData = {
  contactName: string;
...
export function buildContactAlertEmail(data: ContactAlertEmailData): {
  subject: string;
  html: string;
  text: string;
} {
  ...
  const subject = `[TrekSafe] Alerta: ${data.hikerFullName} no confirmó su retorno`;

  const text = [
    `Hola ${data.contactName},`,
    '',
    `TrekSafe registra una alerta de seguridad para ${data.hikerFullName}.`,
    `No se recibió el check-in de retorno dentro del plazo declarado.`,
    '',
    `Destino: ${data.endLocation}`,
    `Punto de inicio: ${data.startLocation}`,
...
<body style="font-family:system-ui,sans-serif;line-height:1.5;color:#0c1a10;max-width:560px;margin:0 auto;padding:24px;">
  ...
  <div style="border:1px solid rgba(12,26,16,0.1);border-top:none;padding:24px;border-radius:0 0 12px 12px;background:#fff;">
    <p>Hola <strong>${escapeHtml(data.contactName)}</strong>,</p>
    <p style="background:#fef2f2;border-left:4px solid #dc2626;padding:12px 16px;margin:16px 0;">
      <strong>${escapeHtml(data.hikerFullName)}</strong> no confirmó su retorno seguro dentro del plazo declarado.
    </p>
    <table style="width:100%;border-collapse:collapse;font-size:14px;">
      <tr><td style="padding:8px 0;color:#587060;">Destino</td><td style="padding:8px 0;"><strong>${escapeHtml(data.endLocation)}</strong></td></tr>
      <tr><td style="padding:8px 0;color:#587060;">Inicio</td><td style="padding:8px 0;">${escapeHtml(data.startLocation)}</td></tr>
      <tr><td style="padding:8px 0;color:#587060;">Retorno estimado</td><td style="padding:8px 0;">${escapeHtml(returnLabel)}</td></tr>
      <tr><td style="padding:8px 0;color:#587060;">Límite (con tolerancia)</td><td style="padding:8px 0;">${escapeHtml(deadlineLabel)}</td></tr>
    </table>
