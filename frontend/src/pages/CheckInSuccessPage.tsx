...
export function CheckInSuccessPage() {
  ...
  const checkedInLabel = new Date(state.checkedInAt).toLocaleString('es-PE', {
    day: 'numeric',
    month: 'short',
    hour: '2-digit',
    minute: '2-digit',
      ...
      <div className="w-full bg-card rounded-2xl border border-border p-5 mb-8 text-left space-y-3">
        {state.endLocation && (
          <div className="flex justify-between gap-3">
            ...
            <span className="text-sm font-semibold text-right">{state.endLocation}</span>
          </div>
        )}
        <div className="flex justify-between gap-3">
          <span className="text-sm text-muted-foreground">Hora de check-in</span>
          <span className="text-sm font-semibold">{checkedInLabel}</span>
        </div>
        <div className="flex justify-between gap-3">
